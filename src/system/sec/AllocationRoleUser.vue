<template>
  <el-dialog :title="role.name" :visible.sync="visible" width="80%" top="5vh"
             @open="openDialog"
             @close="confirm"
            ...
            <section slot="content" style="padding: 5px;" v-loading="tableLoading">
              ...
              <el-table :data="tableData" border highlight-current-row style="width: 100%">
                ...
                <el-table-column label="操作" width="130">
                  <template slot-scope="scope">
                    <el-button v-if="!scope.row.hasRole" type="text" size="small" @click="allocation(scope.row)">
                      分配到角色
                    </el-button>
                  </template>
                </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      allocation(user) {
        this.$http.post('/api/role/rel/user/save/' + this.role.id + '/' + user.id).then(resp => {
          if (resp.success) {
