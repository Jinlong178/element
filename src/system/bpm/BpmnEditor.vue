<template>
  <section class="bpmn-editor" v-loading="loading">
    <!--  侧边工具  -->
    <div class="bmpn-palette" :style="toolsHeight">
    ...
    <!-- 绘图区域 -->
    <div :style="{position: 'relative', height: mainHeight, width: '100%'}">
      <!-- 绘制区域  -->
      ...
      <process-element :element="element" :store="store" :dialog-style="dialogStyle"
                       :update-label-name="updateLabelName"/>
      <process-element v-show="processElementRef != null && !collapse && !showTriggerSettings"
      ...
      <el-button class="settings-btn" v-if='element != null && collapse == false' size="medium"
                 :title="collapse ? '展开配置面板' : '折叠配置面板'"
                 :type="collapse ? 'default' : 'text'"
                 @click="collapse = !collapse"
                 :icon="collapse ? 'el-icon-arrow-left' : 'el-icon-close'"></el-button>

    </div>

    <el-button class="bpmn-btn-item" size="small" @click="download('xml')" title="下载 BPMN2.0 XML"
               :style="{left: 163 +'px'}"
               icon="el-icon-download"></el-button>
    <el-button class="bpmn-btn-item" size="small" @click="download('svg')" title="下载 SVG"
               :style="{left: 153 + 45 +'px'}"
               icon="el-icon-picture-outline"></el-button>
    <el-button class="bpmn-btn-item" size="small" @click="deployment(false)" title="保存"
               :style="{left: 153 + 45 * 2 +'px'}"
               icon="el-icon-check"></el-button>
    <el-button class="bpmn-btn-item" size="small" @click="deployment(true)" type="primary" title="保存并部署"
               :style="{left: 153 + 45 * 3 +'px'}"
               icon="el-icon-upload"></el-button>

    <el-button class="bpmn-btn-item" size="small" @click="confirmBack" title="返回上一页"
               :style="{right: 2 + 'px', width: '70px'}"
               icon="el-icon-back"></el-button>
    <el-button class="bpmn-btn-item" size="small" @click="triggerTriggerDialog"
               :type="showTriggerSettings ? 'primary' : 'default'" title="触发定义"
               :style="{right: 72 + 2 +'px'}"
               icon="el-icon-copy-document"></el-button>
    <transition name="el-zoom-in-center">
      <el-button class="bpmn-btn-item" size="small" v-if="collapse && !showTriggerSettings" @click="collapse = false"
                 type="primary" title="展开配置面板"
                 :style="{right: 72 + 45 + 2 +'px'}"
                 icon="el-icon-setting"></el-button>
    </transition>

    <!-- 消息、信号、错误配置 -->
    <div v-if="showTriggerSettings" class="bpmn-trigger-settings" :style="triggerStyle">
      <el-tabs type="border-card">
        <el-tab-pane label="信号">
          <el-input size="small" v-model="newSignalName">
            <el-select slot="prepend" v-model="scope" placeholder="全局信号">
              <el-option key="global" label="全局信号" value="global"/>
              <el-option key="processInstance" label="当前实例" value="processInstance"/>
            </el-select>
    ...
    <el-button class="settings-btn" v-if='showTriggerSettings' size="medium"
               title="关闭配置面板"
               type="text"
               @click="triggerTriggerDialog"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      triggerStyle() {
        return {
          maxHeight: 'calc(' + this.mainHeight + ' - 40px)'
        }
      },
      dialogStyle() {
        return {
              ...
              for (const processItem of processes) {
                ...
                for (const n of nodeArr) {
                  if (n.type === 'UserTask') {
                    ...
                    const d = n.data
                    if ((d.assigneeFn == null || d.assigneeFn.left == 0) && (d.assignees == null || d.assignees.length == 0)) {
                      this.loading = false
                    ...
                    if (n.type === 'ServiceTask') {
                      ...
                      if (d.executionFn == null || d.executionFn.trim().length == 0) {
                        this.loading = false
                        this.$message.error(n.name + ' 服务任务执行类不能为空')
                        return
                      }
                    }
                  }
